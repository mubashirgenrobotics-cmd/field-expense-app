...
const Button = ({ children, onClick, variant = "primary", style, small, disabled }) => {
  const base = { border: "none", borderRadius: 8, cursor: "pointer", padding: small ? "6px 12px" : "10px 20px", fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6, opacity: disabled ? 0.5 : 1 };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 3000, padding: 16 }}>
      <Card style={{ width: "100%", maxWidth: 400 }}>
        ...
        <p style={{ fontSize: 13, color: "#6B7280" }}>Original Request: <strong>{fmt(item.originalAmount || item.amount)}</strong></p>
        <div style={{ marginBottom: 16 }}><label>Final Approved Amount (₹)</label><input type="number" value={amount} onChange={e => setAmount(e.target.value)} style={{ width: "100%", padding: 8, borderRadius: 8 }} /></div>
        <div style={{ marginBottom: 16 }}><label>Admin Note</label><textarea value={comment} onChange={e => setComment(e.target.value)} style={{ width: "100%", padding: 8, borderRadius: 8 }} /></div>
...
export default function App() {
  ...
  const handleAction = async (id, status, finalAmount, comment) => {
    const col = reviewItem.type === 'expense' ? 'expenses' : 'requests';
    await updateDoc(doc(db, col, id), { status, amount: finalAmount, adminComment: comment, isEdited: finalAmount !== reviewItem.originalAmount });
    setReviewItem(null);
  };

  const handleLogout = () => { localStorage.removeItem("activeUser"); setUser(null); };

  if (!user) return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><Card><h2>Sign In</h2><select onChange={e => { const u = USERS.find(x => x.id === e.target.value); setUser(u); localStorage.setItem("activeUser", JSON.stringify(u)); }}><option>Select User</option>{USERS.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}</select></Card></div>;
  ...
  return (
    <div style={{ padding: 24, fontFamily: "sans-serif" }}>
      <h1>FieldExpense Pro</h1>
      <div style={{ background: "#f0f0f0", padding: 15, borderRadius: 10 }}>
        <p>User: {user.name} | Balance: {fmt(approvedFunds - approvedExpenses)}</p>
        <Button variant="ghost" onClick={handleLogout}>Sign Out</Button>
      </div>
      
      <h2 style={{marginTop: 30}}>{isAdmin ? "Admin Control Panel" : "My Requests & Expenses"}</h2>
      
      {[...requests, ...expenses].filter(i => isAdmin || i.engineerId === user.id).map(i => (
        <Card key={i.id} style={{ margin: "10px 0" }}>
          <p>{i.type.toUpperCase()}: {fmt(i.amount)} | Status: {i.status}</p>
          ...
          {isAdmin && i.status === "pending" && <Button onClick={() => setReviewItem(i)}>Review</Button>}
        </Card>
      ))}

      {reviewItem && <AdminReviewModal item={reviewItem} onClose={() => setReviewItem(null)} onAction={handleAction} />}
    </div>
  );
}