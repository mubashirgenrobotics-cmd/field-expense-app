...
// Compress images from mobile cameras so they fit in Firestore (under 1MB)
...
// ─── ADMIN REVIEW MODAL (Edit Amounts & Approve/Reject) ───────────────────────
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 3000, padding: 16 }}>
      <Card style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
          ...
          <Button onClick={() => { onReject(item.id); onClose(); }} variant="danger" style={{ flex: 1 }}>Reject</Button>
          <Button onClick={() => { onApprove(item.id, parseFloat(amount)); onClose(); }} variant="success" style={{ flex: 1 }}>Approve</Button>
        </div>
      </Card>
    </div>
  );
...
function ExpenseForm({ user, availableBalance, onSubmit, onClose, editItem }) {
  ...
  const handleFile = async (file) => {
    ...
    // Compress if image, otherwise standard base64
  ...
  const submit = () => {
    ...
    onSubmit({
      ...
      type: "expense", status: "pending", // Expenses start as pending now!
    });
    onClose();
  };
...
// ─── LIST COMPONENTS ──────────────────────────────────────────────────────────
function ExpenseList({ expenses, onEdit, onViewAttachment, onReview, isAdmin, filter }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {filtered.map(exp => {
        ...
        return (
          <div key={exp.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px", background: "#FAFAFA", borderRadius: 12, border: "1px solid #F3F4F6" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {filtered.map(req => {
        ...
        return (
          <div key={req.id} style={{ padding: "14px 16px", background: "#FAFAFA", borderRadius: 12, border: "1px solid #F3F4F6", display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}><span style={{ fontWeight: 700, fontSize: 15, color: "#1E40AF" }}>{fmt(req.amount)}</span><Badge status={req.status} /></div>
        ...
        return (
          <Card key={eng.id} style={{ padding: 16 }}>
            ...
            <div style={{ fontSize: 12, display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#6B7280" }}>Spent (Approved)</span><span style={{ color: "#EF4444", fontWeight: 700 }}>{fmt(spent)}</span></div>
...
export default function App() {
  ...
  const [reviewItem, setReviewItem] = useState(null); // Holds request or expense for Admin Review
  ...
  useEffect(() => {
    const unsubExp = onSnapshot(collection(db, "expenses"), (snap) => setExpenses(snap.docs.map(doc => doc.data())));
    const unsubReq = onSnapshot(collection(db, "requests"), (snap) => setRequests(snap.docs.map(doc => doc.data())));
  ...
  // Ledger Math: Only deduct approved AND pending expenses from available balance to prevent overspending
  const approvedFunds = myRequests.filter(r => r.status === "approved").reduce((s, r) => s + r.amount, 0);
  const activeExpenses = myExpenses.filter(e => e.status !== "rejected").reduce((s, e) => s + e.amount, 0);
  const availableBalance = approvedFunds - activeExpenses;
  ...
  const approveItem = async (col, id, finalAmount) => {
    await updateDoc(doc(db, col, id), { status: "approved", amount: finalAmount });
  };
  const rejectItem = async (col, id) => {
    ...
    <div style={{ minHeight: "100vh", background: "#F8FAFC", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
        {tab === "dashboard" && (
          <>
            {isAdmin ? (
              <>
                <div style={{ marginBottom: 24 }}><h2 style={{ margin: "0 0 4px", fontSize: 22, fontWeight: 800 }}>Admin Dashboard</h2></div>
              ...
              <>
                ...
                <Card style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12 }}>
                    {[
                      ...
                      { label: "Active Expenses", value: fmt(activeExpenses), color: "#EF4444", icon: "🧾" },
                ...
                <Card>
                  ...
                  <ExpenseList expenses={myExpenses.slice(0, 5)} onEdit={e => { setEditExpense(e); setShowExpenseForm(true); }} onViewAttachment={setViewAttachment} filter={{ period: "all", status: "all", engineer: "" }} />
                </Card>
              </>
            )}
          </>
        ...
        {tab === "expenses" && (
          <>
            ...
            {filterUI}
            <Card><ExpenseList expenses={isAdmin ? expenses : myExpenses} onEdit={e => { setEditExpense(e); setShowExpenseForm(true); }} onViewAttachment={setViewAttachment} onReview={exp => setReviewItem(exp)} isAdmin={isAdmin} filter={{ status: filterStatus, period: filterPeriod, engineer: filterEngineer }} /></Card>
          </>
        )}
      </div>
      ...
      {reviewItem && (
        <AdminReviewModal 
          ...
          onApprove={(id, amt) => approveItem(reviewItem.type === 'expense' ? 'expenses' : 'requests', id, amt)} 
          onReject={(id) => rejectItem(reviewItem.type === 'expense' ? 'expenses' : 'requests', id)} 
        />
      )}
